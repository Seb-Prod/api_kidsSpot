 ...
 *
 * Gère les requêtes HTTP DELETE. Le corps de la requête doit contenir un JSON avec l'id du favoris à supprimer. Retourne un statut HTTP et un message JSON indiquant le succès ou l'échec de l'opération, ainsi que les erreurs de validation si nécessaire.
 *
 */
...
if ($_SERVER['REQUEST_METHOD'] == 'DELETE') {
    ...
    if (empty($erreurs)) {
        ...
        if (!$favoris->alreadyExists()) {
            ...
            echo json_encode(["message" => "Ce favoris n'existe pas."]);
            exit;
        }
